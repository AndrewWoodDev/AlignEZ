...
if __name__ == '__main__':
    ...
    device = "cuda"
    if 'humor' not in preference:
        path = "RLHFlow/ArmoRM-Llama3-8B-v0.1"
        model = AutoModelForSequenceClassification.from_pretrained(path, device_map="cuda", 
                                trust_remote_code=True, torch_dtype=torch.bfloat16)
        tokenizer = AutoTokenizer.from_pretrained(path, use_fast=True)
    else:
        model = AutoModelForSequenceClassification.from_pretrained('mohameddhiab/humor-no-humor', device_map="cuda", 
                               trust_remote_code=True, torch_dtype=torch.bfloat16)
        tokenizer = AutoTokenizer.from_pretrained('mohameddhiab/humor-no-humor', use_fast=True)

    outdir = f'single_llama32_20_{preference}/roboemb_mlp'
    ...
    chat_outdir = f'llama32_chat_{preference}'
    chat_outputs = []
    for current_idx in range(start_idx, end_idx):
        try:
    ...
    for q, v, o, c in tqdm(zip(questions, vanila_outputs, ours_outputs, chat_outputs)):
        ...
        if 'humor' not in preference:
            ...
            if 'helpful' in preference:
                vanilla_rewards.append(r_vanilla[0])
                ours_rewards.append(r_ours[0])
                chat_rewards.append(r_chat[0])
            else:
                vanilla_rewards.append(r_vanilla[1])
                ours_rewards.append(r_ours[1])
                chat_rewards.append(r_chat[1])

        else:
            humor_reward_vanilla = get_humor_rewards(model, tokenizer, vanilla)
            humor_reward_ours = get_humor_rewards(model, tokenizer, ours)
            humor_reward_chat = get_humor_rewards(model, tokenizer, chat)

            vanilla_rewards.append(humor_reward_vanilla)
            ours_rewards.append(humor_reward_ours)
            chat_rewards.append(humor_reward_chat)

...
def get_insights(df):
    ...
    i = 0
    for input_data, group in grouped:
        dict_q[i] =  {'question': input_data, 'pos': [], 'neg': []}
        pos = []
        neg = []
        for _, row in group.iterrows():
            if row['label'] == 1:
                if isinstance(row['insight'], str):
                    pos.append(input_data.strip().rstrip() + " " + row['insight'].split("\nHuman:")[0])
            elif row['label'] == 0:
                if isinstance(row['insight'], str):
                    neg.append(input_data.strip().rstrip() + " " + row['insight'].split("\nHuman:")[0])
        dict_q[i]['pos'] = pos
        dict_q[i]['neg'] = neg
        i += 1
...
def find_subspace_intersection(subspaces):
    """
    Find the intersection of multiple subspaces.
    
    Args:
        subspaces: List of matrices where each matrix's rows are basis vectors
        
    Returns:
        Tuple of (basis vectors of intersection, dimension of intersection)
    """
    # Stack all basis vectors vertically
    all_vectors = np.vstack(subspaces)
    
    # Find the null space of the stacked matrices
    # This gives us vectors that satisfy all constraints simultaneously
    _, singulars, vh = np.linalg.svd(all_vectors, full_matrices=True)
    
    # Determine numerical rank using a threshold
    tol = 1e-10 * singulars[0]
    rank = np.sum(singulars > tol)
    
    # Get basis vectors for the null space
    null_space = vh[rank:].T
    
    # If null space is empty, intersection is just the zero vector
    if null_space.size == 0:
        return np.zeros((1, all_vectors.shape[1])), 0
    
    return null_space, null_space.shape[1]

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--model-input_data", type=str, required=True)
    ...
    if 'llama-3.1' in model_name.lower():
        outdir=f'single_llama31_{n_layers_to_edit}_{preference}'
    elif 'llama-3.2' in model_name.lower():
        outdir=f'single_llama32_{n_layers_to_edit}_{preference}'
    ...
    for q_idx in tqdm(dict_q):
        ...
        for layer_idx in tqdm(range(n_layers)):
            ...
            for p in preferences:
                # print(p)
                V_reduced_p = np.load(f'cache_ric/{model_name}_neg_{p}_svd/layer_{layer_idx}.npy')[:, :10]
                V_reduced.append(V_reduced_p.T)
            V_reduced, _ = find_subspace_intersection(V_reduced)

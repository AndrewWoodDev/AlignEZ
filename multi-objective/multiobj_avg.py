...
def distance_based_orthogonalize(input_vector, vectors, flag):
    ...
    result1 = input_vector.clone()
    set_1 = vectors[:,:10]
    set_2 = vectors[:,10:20]
    set_3 = vectors[:,20:]
    for i, v in enumerate(set_1.T):
        v = v.unsqueeze(1) if v.dim() == 1 else v
        if len(v.shape) == 0:
            continue
        # Calculate alignment
        alignment = torch.matmul(v.T, result1).squeeze()
        m = torch.nn.Tanh()
        scaling = m(alignment)
        result1 += scaling * v
    
    result2 = input_vector.clone()
    for i, v in enumerate(set_2.T):
        v = v.unsqueeze(1) if v.dim() == 1 else v
        if len(v.shape) == 0:
            continue
        alignment = torch.matmul(v.T, result2).squeeze()
        m = torch.nn.Tanh()
        scaling = m(alignment)
        result2 += scaling * v
    
    result3 = input_vector.clone()
    for i, v in enumerate(set_3.T):
        v = v.unsqueeze(1) if v.dim() == 1 else v
        if len(v.shape) == 0:
            continue
        alignment = torch.matmul(v.T, result3).squeeze()
        m = torch.nn.Tanh()
        scaling = m(alignment)
        result3 += scaling * v

    result = torch.mean(torch.hstack((result2,result1,result3)), dim = 1)
...
if __name__ == '__main__':
    ...
    preferences = ['humor', 'harmless', 'helpful']
    if 'llama-3.1' in model_name.lower():
        outdir=f'llama31_multi_nlayers_{n_layers_to_edit}_{preference}'
    elif 'llama-3.2' in model_name.lower():
        outdir=f'avg_llama32_ric_{"_".join(preferences)}_nlayers_{n_layers_to_edit}_{preference}'
    ...
    insights_file = f'multipref_llama32_generated_data/filtered_helpful_insights.csv'
    ...
    for q_idx in tqdm(dict_q):
        ...
        for layer_idx in tqdm(range(n_layers)):
            ...
            for p in preferences:
                V_reduced_p = np.load(f'cache_ric/{model_name}_{p}_svd/layer_{layer_idx}.npy')[:, :10]
                V_reduced.append(V_reduced_p.T)
            V_reduced = np.vstack(V_reduced).T
